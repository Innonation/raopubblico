...
from agency.models import IdentityRequest, TokenUser
from agency.utils.utils import format_id_card_issuer, encrypt_data, set_client_ip
from agency.utils.utils_api import sign_token_api, sign_token_hmac_api
...
def signed_token(user, op_username, rao, pin=None, api_data=None):
    """
    Creazione token_sigillato
    :param api: modifica il flusso se chiamata da API o no
    :param user: dict contenente i dati dell'utente
    :param op_username: username dell'operatore
    :param pin: pin dell'operatore
    :param rao: oggetto RAO del database
    :return: dict con statusCode, token e passphrase
    """
    creation_time = datetime.datetime.utcnow()
    expiration_time = creation_time + datetime.timedelta(days=30)

    request_identity = IdentityRequest.objects.filter(fiscalNumberUser=user['fiscalNumber'].upper()).order_by(
        'timestamp_identification').last()

    token_to_update = TokenUser.objects.filter(uuid_token_user=request_identity.token_id).last()
    ...
    try:
        if api_data:
            dict_signature = sign_token_hmac_api(op_username, payload, api_data)
        else:
