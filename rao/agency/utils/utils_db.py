            ...
            if status_code_reset == StatusCode.OK.value:
                ...
                if status_code_disable == StatusCode.OK.value:
                    ...
                    if status_email == StatusCode.OK.value:
                        ...
                        LOG.warning("{}, {} - Errore durante l'invio mail di attivazione".format(
...
def update_emailrao(op, rao_name, rao_email, rao_host, rao_pwd, email_crypto_type, email_port, smtp_mail_from=""):
    ...
    try:
        ...
        if not entry_rao:
            ...
            tmp_settings = TempMailSettings(smtp_mail_from, rao_email, rao_host, password=password,
...
def change_setup_value(request, t):
    ...
    try:
        ...
        if settings_rao and operator:
            ...
            params_t[
                'alert_message'] = "Dati modificati correttamente. Inserisci tutti i campi riportati nella form sottostante per proseguire con l'attivazione."
            t = signing.dumps(params_t)
        else:
            params_t = signing.loads(t)
            ...
            t = signing.dumps(params_t)
            LOG.error("Codice Fiscale e/o Codice IPA non modificati. Dati su Rao non trovati.",
                      extra=agency.utils.utils.set_client_ip(request))
...
def get_log_idr_by_date(day):
    """

    :param day:
    :return:
    """
    date_day = datetime.datetime.strptime(day, "%Y-%m-%d").date()
    idrs = IdentityRequest.objects.filter(timestamp_identification__date=date_day)

    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="idr-{}.csv"'.format(date_day)
    writer = csv.DictWriter(response, fieldnames=['Token ID', 'Operatore', 'CF Cittadino', 'Data Identificazione'])
    writer.writeheader()

    for idr in idrs:
        writer.writerow(
            {'Token ID': idr.token_id, 'Operatore': idr.idOperator.fiscalNumber, 'CF Cittadino': idr.fiscalNumberUser,
             'Data Identificazione': idr.timestamp_identification.strftime("%Y-%m-%d %H:%M")})
