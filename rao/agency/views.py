...
from .utils.utils_mail import send_email
...
from .utils.utils_db import get_all_operator, get_attributes_RAO, update_password_operator, \
    ...
    update_sign_field_operator, update_status_operator, update_emailrao, \
    update_is_activated_field_operator
from .utils.utils_setup import configuration_check, init_settings_rao, init_user
from .utils.utils_token import signed_token
                ...
                if not username or not password:
                    ...
                    if result in (StatusCode.OK.value, StatusCode.EXPIRED_TOKEN.value, StatusCode.FORBIDDEN.value):
                        ...
                        if result == StatusCode.EXPIRED_TOKEN.value:
                            ...
                            LOG.info("{} - Credenziali dell'utente scadute".format(username),
...
def add_identity(request, t):
    ...
    try:
        ...
        messages = None
        form = NewIdentityForm(username=request.session['username'])
        if request.method == 'POST':
            if 'identifica' not in request.POST:
                form = NewIdentityForm(request.POST, username=request.session['username'])
            else:
                form = NewIdentityPinForm(request.POST, username=request.session['username'])
    ...
    try:
        ...
        if request.session['identified']:
            ...
            if 'user_detail' in params:
                ...
                pin = params['pin']
                dict_token = signed_token(identity, request.session['username'], get_attributes_RAO(), pin)
...
def change_pin(request, t):
    ...
    try:
        ...
        if 'alert_type' in params_t and 'alert_message' in params_t:
            messages = display_alert(AlertType.DANGER, params_t.pop('alert_message')) if params_t.pop(
                'alert_type') == 'danger' \
            ...
            if form.is_valid():
                if 'is_admin' in params_t and not configuration_check():
                    ...
                    if result == StatusCode.OK.value:
                        ...
                        LOG.warning("{} - Nuova password uguale alla precedente.".format(username),
                                    extra=set_client_ip(request))
                        messages = display_alert(AlertType.DANGER,
        ...
        if request.POST:
            if 'update_cert' in request.POST:
                ...
                if form_email.is_valid():
                    ...
                    is_updated = update_emailrao(active_operator, get_attributes_RAO().name, rao_email, rao_host,
                                                 rao_pwd,
                    ...
                    if not is_updated:
                        ...
                        return render(request, settings.TEMPLATE_URL_AGENCY + 'setup.html',
                                      {'form_email': form_email, 'messages': messages, 'params': params, 'token': t})
                    LOG.info("{}, {} - Configurazione SMTP modificata.".format(
                        agency.utils.utils_db.get_attributes_RAO().issuerCode,
                        request.session['username']), extra=set_client_ip(request))
                    return HttpResponseRedirect(reverse('agency:list_identity', kwargs={'t': t, 'page': 1}))
